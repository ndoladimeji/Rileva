...
for fold in range(5):
    ...
    train_data = train_folds[train_folds['kfold'] != fold]
    val_data = train_folds[train_folds['kfold'] == fold]

    # Split the features and labels for training and validation sets
    X_train = train_data.drop(['target', 'kfold'], axis=1)  # Adjust column names if needed
    y_train = train_data['target']
    X_val = val_data.drop(['target', 'kfold'], axis=1)  # Adjust column names if needed
    y_val = val_data['target']

    # Perform the training and validation using the algorithm/model
    # Train the algorithm/model on the training set (X_train, y_train)
    # Validate the algorithm/model on the validation set (X_val, y_val)
    ...
    # Example code to train and validate the model
